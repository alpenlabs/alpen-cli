...
pub struct SettingsFromFile {
    ...
    /// Number of addresses cached beyond the last known address during Bitcoin Core's initial
    /// full recovery scan. Defaults to 50 and must be greater than 0. It has no effect after the
    /// first full scan or when using the Esplora backend.
    pub recovery_lookahead: Option<u32>,
    /// Number of consecutive unused reclaim-key counters that end seed recovery. Defaults to 50
    /// and must be greater than 0. Smaller values stop sooner but can miss deposits after fewer
    /// unused counters and require more Bitcoin Core full-chain scans.
...
pub struct Settings {
    ...
    /// Addresses cached beyond the last known address during Bitcoin Core's initial full scan.
    pub recovery_lookahead: u32,
    /// Consecutive unused reclaim-key counters that end seed recovery.
...
fn resolve_seed_recovery_gap_limit(configured: Option<u32>) -> Result<u32, ConfigError> {
    ...
    if gap_limit == 0 {
        return Err(ConfigError::Message(
            "seed_recovery_gap_limit must be greater than 0".to_owned(),
        ));
    }
    Ok(gap_limit)
}
...
mod tests {
    ...
    #[test]
    fn test_seed_recovery_gap_limit_defaults_to_50_and_rejects_zero() {
        ...
        assert_eq!(resolve_seed_recovery_gap_limit(Some(1)).unwrap(), 1);
